...
class Solution(object):
    def validateCoupons(self, code, businessLine, isActive):
        ...
        for cod, bus, status in zip(code, businessLine, isActive):
            if re.match(r"^[a-zA-Z0-9_]+$", cod) and bus in priority and status:
                valid_coupons.append((bus, cod))

        # Sort first by businessLine priority, then by code lexicographically
        valid_coupons.sort(key=lambda x: (priority[x[0]], x[1]))
        return [cod for _, cod in valid_coupons]
